...
def parse_chain_from_core(core: str) -> Optional[str]:
    """
    For this project, the config name does NOT encode a chain ID.
    Return None so we auto-pick the chain from the PDB (by C3' count).
    """
    return None
...
def pdb_stem_from_core(core: str) -> Optional[str]:
    """
    Map a LAMMPS core like:
        config_01_len50_01.dat
    to the PDB stem:
        seq1_rank_1

    i.e., take the first numeric field (01 -> 1) and always use rank_1.
    """
    m = re.match(r"config_(\d+)_len(\d+)_\d+\.dat$", core)
    if not m:
        return None

    seq_idx = int(m.group(1))  # "01" -> 1
    # length = int(m.group(2))  # 50, if you ever need it
    return f"seq{seq_idx}_rank_1"
