...
Run from your outputs folder:
  ...
  python analyze_all_ns.py --pdb-dir "." --chain A --exclude-res 4A
...
- NEW: You can exclude specific PDB residues (e.g., --exclude-res 4A,15,120B)
       or whole insertion-code classes (e.g., --exclude-icode A) from the C3′
       reference used for RMSD.
"""
...
def parse_args():
    ...
    ap.add_argument("--chain", help="Preferred chain ID (e.g., A). If missing or empty, script will auto-pick the best chain.")
    ...
    ap.add_argument("--clip", type=float, help="Discard data AFTER this value for plotting only, in the chosen --time-unit.")

    # NEW: Exclusions for the PDB reference (affects RMSD only)
    ap.add_argument("--exclude-res", default="",
                    help="Comma list of PDB residues to exclude (number with optional insertion code), e.g. '4A,15,120B'.")
    ap.add_argument("--exclude-icode", default="",
                    help="Comma list of insertion codes to exclude from the reference, e.g. 'A,B'.")
...
def c3_coords_sorted_filtered(
    pdb_path: Path,
    chain: str,
    ex_nums: Optional[Set[int]] = None,
    ex_pairs: Optional[Set[Tuple[int,str]]] = None,
    ex_icodes: Optional[Set[str]] = None,
) -> np.ndarray:
    """Return Nx3 array of C3' coords for a chain, excluding requested residues."""
    ex_nums = ex_nums or set()
    ex_pairs = ex_pairs or set()
    ex_icodes = {c.upper() for c in (ex_icodes or set())}
    ...
    for ln in open_text_auto(pdb_path):
        ...
        atn = ln[12:16].strip()
        if atn not in ("C3'", "C3*") and atn.upper() not in ("C3'", "C3*"):
            continue

        resseq = ln[22:26].strip()
        icode = ln[26].strip().upper()  # insertion code in column 27

        try:
            rnum = int(resseq)
        except Exception:
            # Non-integer residue numbers are ignored
            continue

        # Exclusion rules
        if rnum in ex_nums:
            continue
        if (rnum, icode) in ex_pairs:
            continue
        if icode and icode in ex_icodes:
            continue
...
def main():
    ...
    # Parse PDB residue exclusions
    ex_nums, ex_pairs, ex_icodes = set(), set(), set()
    if args.exclude_res:
        for tok in args.exclude_res.split(","):
            tok = tok.strip()
            if not tok:
                continue
            ...
            if not m:
                continue
            num = int(m.group(1))
            code = m.group(2).upper() if m.group(2) else ""
            if code:
                ex_pairs.add((num, code))
            else:
                ex_nums.add(num)
    if args.exclude_icode:
        ex_icodes = {s.strip().upper() for s in args.exclude_icode.split(",") if s.strip()}
    ...
    for traj in trajs:
        ...
        # Build reference (auto-pick chain if needed), applying exclusions
        ...
        if pdb_path and pdb_path.exists():
            ...
            if used_chain and c3map.get(used_chain, 0) > 0:
                ref = c3_coords_sorted_filtered(pdb_path, used_chain, ex_nums, ex_pairs, ex_icodes)
            else:
                if c3map:
        ...
        if args.verbose:
            print(f"[REF] core={core} preferred={preferred} used={used_chain} "
                  f"C3'(pre-excl?) counts={c3map} "
                  f"ref_after_excl={(0 if ref is None else ref.shape[0])}")
        ...
        dbg.write(f"ref_C3prime_after_exclude_N={(0 if ref is None else Nref)}\n")
        dbg.write(f"exclude_res_pairs={sorted(list(ex_pairs))}\n")
        dbg.write(f"exclude_res_nums={sorted(list(ex_nums))}\n")
        dbg.write(f"exclude_icodes={sorted(list(ex_icodes))}\n")
        ...
        summary.append((traj.name, f"chain={used_chain}", f"C3'={(0 if ref is None else Nref)}",
                        f"frames={len(times_ns)}", f"finite_RMSD={good_frames}"))
