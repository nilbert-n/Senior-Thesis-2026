...
and Potential Energy vs time (from LAMMPS logs). Time axis is in **nanoseconds**.
...
Outputs in the chosen --outdir (default: current dir):
  ...
  metrics/metrics_<config>.csv   (time_ns, Rg, RMSD)
  metrics/pe_<config>.csv        (time_ns, Pe)
...
def parse_args():
    import argparse
    ap = argparse.ArgumentParser(description="Rg/RMSD(to C3')/PE vs time (ns), robust PDB+log discovery.")
    ...
    ap.add_argument("--traj-glob", default="config_*.dat_SA.lammpstrj", help="Glob for many trajectories (default matches your files).")
...
def main():
    ...
    def find_log(core: str, tdir: Path) -> Optional[Path]:
        candidates = [
            ...
            tdir / "outputs_sa" / f"log.{core}.lammps",
            tdir.parent / "outputs_sa" / f"log.{core}.lammps",
        ]
        for c in candidates:
    ...
    for traj in trajs:
        ...
        # Plots
        plt.figure(); plt.plot(times_ns, rgs); plt.xlabel("Time (ns)"); plt.ylabel("Radius of gyration (Rg)"); plt.title(f"Rg vs time — {core}")
        plt.savefig(plots / f"Rg_{core}.png", dpi=150, bbox_inches="tight"); plt.close()

        plt.figure(); plt.plot(times_ns, rmsds); plt.xlabel("Time (ns)"); plt.ylabel("RMSD to PDB C3' (Å)"); plt.title(f"RMSD vs time — {core} (chain {used_chain})")
        plt.savefig(plots / f"RMSD_{core}.png", dpi=150, bbox_inches="tight"); plt.close()

        if steps.size:
            plt.figure(); plt.plot(tns, pe); plt.xlabel("Time (ns)"); plt.ylabel("Potential energy (kcal/mol)"); plt.title(f"PE vs time — {core}")
