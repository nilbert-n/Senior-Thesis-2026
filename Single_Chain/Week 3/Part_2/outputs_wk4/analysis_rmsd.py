#!/usr/bin/env python3
"""
analyze_rmsd_only.py
Compute RMSD (to PDB C3') vs time for one or more trajectories.
...
def parse_args():
    import argparse
    ap = argparse.ArgumentParser(description="RMSD (to PDB C3') vs time with strict filename matching.")
    ap.add_argument("--pdb", help="Explicit C3' PDB path (reference for ALL trajectories). Strongly recommended.")
    ap.add_argument("--pdb-dir", default=".", help="Directory to look for PDBs if --pdb not given (default: '.').")
    ...
    ap.add_argument("--log", help="Explicit log path (.lammps or .gz). Used only to read timestep.")
    ap.add_argument("--timestep-fs", type=float, default=None,
                    help="Override timestep fs (else read from log; fallback --default-timestep-fs).")
    ap.add_argument("--default-timestep-fs", type=float, default=10.0,
                    help="Fallback timestep fs if not in log (default 10).")
    ...
    ap.add_argument("--plot-downsample", type=int, default=0,
                    help="If >0, downsample plotted points to this many.")
...
def find_pdb_for_core(pdb_dir: Path, core: str, allow_gz: bool, verbose=False) -> Optional[Path]:
    ...
    for c in candidates:
        if c.exists():
            if verbose: print(f"[PDB] auto-found {c}")
            return c
...
# ---------------- Log / timestep only ----------------
def parse_log_timestep(log_path: Optional[Path], default_fs: float, verbose=False):
    ts = default_fs
    if not log_path or not log_path.exists():
        if verbose: print(f"[LOG] missing: {log_path}, using default timestep {default_fs} fs")
        return ts
...
def iter_frames(path: Path):
    with open_stream_auto(path) as fh:
        while True:
            ...
            # NUMBER OF ATOMS
            lbl = fh.readline()
            if not lbl or not lbl.strip().startswith("ITEM: NUMBER OF ATOMS"):
                continue
            try:
            ...
            # BOX BOUNDS (skip 3)
            bb = fh.readline()
            if not bb or not bb.strip().startswith("ITEM: BOX BOUNDS"):
                continue
...
def main():
    ...
    for traj in trajs:
        ...
        ts_fs = parse_log_timestep(log_path, ts_default, verbose=args.verbose)
        if args.timestep_fs is not None:
            ts_fs = args.timestep_fs

        # Reference build (C3' only)
        ...
        if pdb_path and pdb_path.exists():
            ...
            if args.verbose:
                print(f"[WARN] PDB not found for {core}; RMSD will be NaN.")
        ...
        if args.verbose:
            ...
            print(f"[REF] preferred_chain={preferred} used_chain={used_chain} "
                  f"C3'={(0 if ref is None else ref.shape[0])} counts={c3map}")
        ...
        for fr in iter_frames(traj):
            ...
            if first_N is None:
                first_N = xyz.shape[0]
                if ref is not None and Nref is not None and Nref > first_N and args.verbose:
                    print(f"[WARN] Ref C3' atoms ({Nref}) > traj atoms ({first_N}); truncating to {first_N}.")
        ...
        # CSV: time + RMSD only
        dfm = pd.DataFrame({"time_ns": times_ns, "RMSD": rmsds})
        dfm.to_csv(metrics / f"metrics_{core}.csv", index=False)

        # Plot RMSD vs time only
        ...
        summary.append((
            traj.name,
            f"chain={used_chain}",
            f"C3'={(0 if ref is None else Nref)}",
            f"frames={len(times_ns)}",
            f"finite_RMSD={good_frames}"
        ))
