...
def analyze_two_tier(
    ...
    # Raw data as a faint background trace for reference
    ax.plot(time_ns, n_in_pocket_per_frame,
            color="#90CAF9", lw=0.8, alpha=0.4, label="_nolegend_")
    # Smooth curve on top
    ax.plot(time_ns, smoothed_occupancy,
            color="#1565C0", lw=2.2, label="Smoothed occupancy")
    ax.axhline(
        mean_occupancy,
        color="#E91E63",
        ls="--",
    ...
    ax.set_title("A  |  Encounter", fontsize=10, fontweight="bold")
    ...
    ax.fill_between(
        ...
        label=f"Site occupied (f_site = {res['f_site']:.4f})"
    )
    ax.set_xlabel("Time (ns)", fontsize=11)
    ax.set_ylabel("Bound State (0/1)", fontsize=11)
    ax.set_ylim(-0.1, 1.3)
    ax.set_title("B  |  Binding Success", fontsize=10, fontweight="bold")
    ...
    ax.set_title("C  |  Dwell Times", fontsize=10, fontweight="bold")
    ax.grid(alpha=0.3)
