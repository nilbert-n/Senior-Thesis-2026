...
Modelling how ASOs bind to mRNA required both a physically stable model of the target RNA and an appropriate simulation environment where ASOs can interact with it. In this chapter, we describe the modeling pipeline. Experimental 3-dimensional structures or NuFold predicted RNA sequences were prepared; coarse-grained molecular dynamics (CGMD) was then used to fold and equilibrate each sequence to determine the regions most likely to bind ASOs by assessing their secondary structure and Solvent-Accessible Surface Area (SASA); finally, ASO binding was quantified based on centre of mass (COM) distance. All simulations were run with LAMMPS~\cite{Thompson2022} as before, using the SIS coarse-grained RNA force field of Aierken and Joseph~\cite{Aierken2024} for RNA folding and binding studies. Subsequently, post-processing was carried out with custom Python code and the tool described above.
%% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
Validation of our simulation parameters was carried out using an existing crystal structure of a CAG-repeat RNA duplex 3NJ6~\cite{Aierken2024}. Because this structure has already been validated against our SIS model’s parameters, we used it as a benchmark to test whether our force field has been properly implemented. In order to create the validation structure, we removed the complementary hydrogen atoms and crystallographic water molecules, leaving a single-stranded chain. The single strand was then used to extract the  C3$^\prime$ atom coordinates that were used as a fixed reference for calculating root mean square deviations (RMSD).
...
In order to predict the secondary structure, we first performed multiple sequence alighnement using rMRSA on the target's FASTA, and the used IPknot to perform secondary structure constraing predictions, and thirdky, Nufold executed using the training configeration preset. The rank-1 output PDB for each sequence represented the predicted three dimensional structure for the corresponding RNA. Each rank-1 PDB was subsequently read in order to obtain its C3$^\prime$ coordinates which would be stored as the fixed RMSD reference for all subsequent simulation runs for that specific RNA target.


\subsection{Longer and mRNA-Scale Sequences}
\label{sec:longer_seqs}

Ten RNA sequences with the compositional criteria for natural occurring m-RNA sequences were randomly generated and sampled (between 50-200 bases). Additionally, sequences of approximately 1000 bases were used to test the ability of the pipeline to scale to mRNA-length chains. All sequence within this length range were folded according to the Simulated Annealing protocol outlined in Section~\ref{sec:sa}, and secondary structures were analyzed with RNALfold as explained in Section~\ref{sec:sec_struct}.
...
ATOM and HETATM records were filtered to retain only recognized RNA and
DNA nucleotide residue names; DNA thymine (T) residues were mapped to
uracil (U) to enforce an RNA alphabet throughout.
...
the NuFold Fig.~5 targets, chain selections were fixed to match the
...
The PDB files for all RNA molecules were analyzed to obtain inforamtion about the RNA residues. All ATOM and HETATM records that represent RNA/DNA nucleotide residue names were filtered out, and T residues, representing DNA thymine, were replaced with U (uracil) for consistent RNA alphabet. For cases where multiple chains existed in the PDB file for example the Nufold Fig.~5 targets, specific chain selections were made based on the assignemnts described in described in Kagaya et al. ~\cite{Kagaya2025}, i.e. chain C for 2DER and chain A for 6CK4. After sorting the residues in increasing order of their sequence numbers, the resulting nucleotide sequence (A, C, G, U) was used as the basis for generating the LAMMPS data files.
...
All simulation methods utilized the SIS coarse-grained RNA force-field that was developed by Nguyen, Hori, and Thirumalai\cite{Aierken2024}, with a simulation pipeline develiped by Aierken and Joseph. Each nucleotide was represented as a bead with interactions that have been parametrized to represent the relative energies associated with each type of standard Watson-Crick base pair interaction (C-G and A-U) and G-U wobble base pair interaction. The total potential energy is given by:
...
The base pairing and stacking interactions in this system are modeled using a custom LAMMPS pair style \texttt{base/rna}~\cite{Aierken2024}, with a cutoff of 18.0~\AA. This pair style represents the anisotropic geometry of Watson-Crick base pairing using both inter-bead distances, four angles, and two dihedral angles to reproduce the A-form helix of RNA. The interaction strengths for each type of base pair were set as follows:
$\varepsilon = -5.0$~kcal\,mol$^{-1}$, A -U and G -U wobble pairs at
$\varepsilon = -3.33$~kcal\,mol$^{-1}$, and all non-complementary pairs
at $\varepsilon = 0$.
...
Each minimized system was run for 50~ns ($5 \times 10^{6}$ time steps and $\Delta t=10$ fs) in the canonical ensemble (NVT), as described above. The ASO simulations were also performed at temperatures of $298$ K and $300$ K.

The NVT ensemble uses the NVE velocity-verlet integration method, coupled with a Langevin thermostat~\cite{Schneider1978}. The thermal coupling constant used by the Langevin thermostat has been set to $\tau = 100,000$ fs.
...
Sampling was done using production runs for both RNA and ASO under NVT conditions, but over different time scales: 1,000~ns for ASO simulations and 4~\textmu s for RNA simulations. 
...
Standard NVT molecular dynamic simulations for nucleic acid 
...
for kinetic trapping during NVT simulation. Thus, simulated annealing was utilized.}
The details of the simulated annealing procedure follow the implementation 
of Aierken and Joseph \cite{Aierken2024}.

We carried out a 10 step heating cycle that consisted of: ramp 298~K$\rightarrow T_\text{high}$ (2.5~ns) $\rightarrow$ hold at $T_\text{high}$ (2.5~ns) $\rightarrow$
cool to 280~K (5~ns) $\rightarrow$ hold at 280~K (2.5~ns);

The convergence of SA trajectories versus NVT runs was evaluated by comparison of
...
\textbf{[TODO: specify the SASA tool (e.g., FreeSASA, MDAnalysis, or
custom Python script), probe sphere radius, and number and spacing of
frames sampled.]}
...
%% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\section{ASO Chemical Modifications: System Variants}
\label{sec:aso_variants}
%% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

\textbf{[TODO: Describe the specific ASO chemical modifications studied
(e.g., 2$^\prime$-OMe, 2$^\prime$-F, PS backbone, LNA), how each
modification was encoded in the coarse-grained topology (different bead
types? modified interaction parameters?), and how the variant data files
were generated from the baseline \texttt{25ASO\_unmod} topology.
Populate once Project~2 materials are available.]}

%% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
\section{ASO Project 2: Extended Binding Analysis}
\label{sec:project2}
%% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

The binding affinity pipeline described in
Section~\ref{sec:binding_affinity} was also applied to a second RNA
target system.
\textbf{[TODO: Populate once Project~2 materials are available.
Required: target RNA sequence and length, ASO variants and modification
states, LAMMPS data file names, any deviations from the standard
protocol, and a table of $K_D$ and $K_A^*$ values.]}