...
def generate_slurm_script(filename, job_name, lammps_input, ntasks=4,
                          walltime="10:00:00", mem_per_cpu="2G"):
    """
...
def main():
    ...
    for n_aso, box_half in conc_sweep.items():
        ...
        # Scale tasks and walltime with system size
        ntasks = 4 if n_aso <= 100 else 8
        wt = "10:00:00" if n_aso <= 100 else "20:00:00"
        generate_slurm_script(sh_path, f"ASO{n_aso}_300K", in_file, ntasks=ntasks, walltime=wt)
